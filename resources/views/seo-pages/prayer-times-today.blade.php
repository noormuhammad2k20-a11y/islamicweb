...
<script type="application/ld+json">
{
  ...
  "mainEntity": [{
    ...
    "acceptedAnswer": {
      "@@type": "Answer",
      "text": "Fajr prayer time today begins at {{ $prayers['fajr'] }}."
    }
  }, {
    "@@type": "Question",
    ...
    "acceptedAnswer": {
      "@@type": "Answer",
      "text": "Maghrib prayer time today begins at {{ $prayers['maghrib'] }}."
    }
  }]
}
</script>
...
<div class="page-header text-center" style="padding: 60px 20px; background: var(--bg-tinted); border-bottom: 1px solid var(--border-light);">
    <div style="max-width: 800px; margin: 0 auto;">
        ...
        <p style="font-size: 1.1rem; color: var(--text-medium); line-height: 1.6;">Accurate Namaz timings for today, <strong>{{ $date->format('d F Y') }}</strong> ({{ $hijri['day'] }} {{ $hijri['month_name'] }} {{ $hijri['year'] }} AH).</p>
    </div>
</div>
...
<div class="container" style="max-width: 1000px; margin: 50px auto; padding: 0 20px;">
    ...
    <div style="background: white; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); padding: 30px; margin-bottom: 40px;">
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 20px; text-align: center;">
            <div style="padding: 20px; background: var(--bg-tinted); border-radius: 8px;">
                ...
                <div style="font-size: 1.5rem; font-weight: bold; color: var(--gold-dark);">{{ $prayers['fajr'] }}</div>
            </div>
            <div style="padding: 20px; background: var(--bg-tinted); border-radius: 8px;">
                <h3 style="color: var(--navy); margin-bottom: 10px;">Sunrise</h3>
                <div style="font-size: 1.5rem; font-weight: bold; color: var(--text-medium);">{{ $prayers['sunrise'] }}</div>
            </div>
            <div style="padding: 20px; background: var(--bg-tinted); border-radius: 8px;">
                <h3 style="color: var(--navy); margin-bottom: 10px;">Dhuhr</h3>
                <div style="font-size: 1.5rem; font-weight: bold; color: var(--gold-dark);">{{ $prayers['dhuhr'] }}</div>
            </div>
            <div style="padding: 20px; background: var(--bg-tinted); border-radius: 8px;">
                <h3 style="color: var(--navy); margin-bottom: 10px;">Asr</h3>
                <div style="font-size: 1.5rem; font-weight: bold; color: var(--gold-dark);">{{ $prayers['asr'] }}</div>
            </div>
            <div style="padding: 20px; background: var(--bg-tinted); border-radius: 8px;">
                <h3 style="color: var(--navy); margin-bottom: 10px;">Maghrib</h3>
                <div style="font-size: 1.5rem; font-weight: bold; color: var(--gold-dark);">{{ $prayers['maghrib'] }}</div>
            </div>
            <div style="padding: 20px; background: var(--bg-tinted); border-radius: 8px;">
                <h3 style="color: var(--navy); margin-bottom: 10px;">Isha</h3>
                <div style="font-size: 1.5rem; font-weight: bold; color: var(--gold-dark);">{{ $prayers['isha'] }}</div>
            </div>
        </div>
    </div>
    ...
    <div style="line-height: 1.8; color: var(--text-dark); margin-bottom: 50px;">
        ...
        <div style="margin-top: 30px; text-align: center;">
            <a href="{{ route('prayer-times.hub') }}" style="display: inline-block; padding: 12px 25px; background: var(--navy); color: white; border-radius: 5px; text-decoration: none; font-weight: bold;">View Timings for Your Specific City</a>
        </div>
    </div>
