...
@section('title', 'نورِ اسلام | Noor-e-Islam')
        ...
        <div class="hero-inner">
            <div class="hero-content">
                ...
                <div class="hero-buttons">
                    <a href="{{ route('islamic-date.hub') }}" class="btn-primary"><i class="fas fa-calendar-day"></i> Today's Date</a>
            ...
            <div class="hero-visual">
                <div class="hero-mosque-container">
                    <div class="hero-mosque-bg">
                        <img src="{{ asset('favicon.svg') }}" alt="Hero Icon" style="width: 85%; height: auto; border-radius: 50%; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
                    </div>
        ...
        <div class="prayer-ticker-inner">
            ...
            <div class="prayer-times-list" id="prayerTimesList">
                ...
                @if($currentPrayerTime)
                    <div class="prayer-time-chip"><span class="prayer-name">Fajr</span><span class="prayer-time-val">{{ \Carbon\Carbon::parse($currentPrayerTime->fajr)->format('h:i A') }}</span></div>
                    <div class="prayer-time-chip"><span class="prayer-name">Sunrise</span><span class="prayer-time-val">{{ \Carbon\Carbon::parse($currentPrayerTime->sunrise)->format('h:i A') }}</span></div>
                    <div class="prayer-time-chip"><span class="prayer-name">Dhuhr</span><span class="prayer-time-val">{{ \Carbon\Carbon::parse($currentPrayerTime->dhuhr)->format('h:i A') }}</span></div>
                    <div class="prayer-time-chip"><span class="prayer-name">Asr</span><span class="prayer-time-val">{{ \Carbon\Carbon::parse($currentPrayerTime->asr)->format('h:i A') }}</span></div>
                    <div class="prayer-time-chip"><span class="prayer-name">Maghrib</span><span class="prayer-time-val">{{ \Carbon\Carbon::parse($currentPrayerTime->maghrib)->format('h:i A') }}</span></div>
                    <div class="prayer-time-chip"><span class="prayer-name">Isha</span><span class="prayer-time-val">{{ \Carbon\Carbon::parse($currentPrayerTime->isha)->format('h:i A') }}</span></div>
                @else
                ...
                <div class="about-image-wrapper">
                    <div class="about-image-main">
                        <div style="background: var(--primary); width: 100%; height: 100%; min-height: 400px; border-radius: 12px; display: flex; align-items: center; justify-content: center;">
                            <img src="{{ asset('favicon.svg') }}" alt="About Icon" style="width: 250px; height: auto; border-radius: 50%; box-shadow: 0 10px 40px rgba(0,0,0,0.2);">
                        </div>
        ...
        <div class="section-inner">
            <div class="section-header" style="margin-bottom: 40px; position: relative;">
                ...
                <div style="margin-top: 20px;">
                    <a href="{{ route('surah.index') }}" class="btn-primary" style="padding: 8px 25px; border-radius: 25px; font-size: 0.95rem; display: inline-flex; align-items: center; gap: 8px;">
            ...
            <div class="surah-grid">
                ...
                <a href="{{ route('surah.show', $surah->slug) }}" class="surah-card">
                    <div class="surah-number">{{ $surah->number }}</div>
                    <div class="surah-name-ur">{{ $surah->name_ur }}</div>
                    <div class="surah-name-en">Surah {{ $surah->name_en }}</div>
                    <div class="surah-meta">
                        <span><i class="fas fa-layer-group"></i> {{ $surah->ayah_count }} Ayahs</span>
                    </div>
        ...
        <div class="section-inner">
            ...
            <div class="services-grid">
                <div class="service-card">
                    ...
                    <a href="{{ route('islamic-date.hub') }}" class="service-link">View Date <i class="fas fa-arrow-right"></i></a>
                </div>
                <div class="service-card">
                    ...
                    <a href="{{ route('prayer-times.hub') }}" class="service-link">View Prayer Times <i class="fas fa-arrow-right"></i></a>
                </div>
                <div class="service-card">
                    ...
                    <a href="{{ route('hadith.index') }}" class="service-link">Read Hadith <i class="fas fa-arrow-right"></i></a>
                </div>
                <div class="service-card">
                    ...
                    <a href="{{ route('converter.show') }}" class="service-link">Use Converter <i class="fas fa-arrow-right"></i></a>
                </div>
                <div class="service-card">
                    ...
                    <a href="{{ route('events.index') }}" class="service-link">View Calendar <i class="fas fa-arrow-right"></i></a>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="section" id="99-names" style="background: var(--secondary-light); padding: 60px 0;">
        <div class="section-inner">
            <div class="section-header" style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 30px;">
                ...
                <a href="{{ route('names_allah.index') }}" class="btn-primary" style="padding: 8px 20px; border-radius: 20px; font-size: 0.9rem;">View All Names</a>
            </div>
            <div style="display: flex; gap: 15px; overflow-x: auto; padding-bottom: 20px; scrollbar-width: none;">
                @foreach($allahNames->take(12) as $name)
                <div style="min-width: 140px; background: var(--white); padding: 25px 15px; border-radius: 12px; text-align: center; border: 1px solid rgba(17, 70, 121, 0.08); box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
                    <div style="font-size: 1.8rem; color: var(--gold); font-family: 'Amiri', serif; margin-bottom: 10px;">{{ $name->arabic }}</div>
                    <div style="font-weight: 700; color: var(--primary); font-size: 1.1rem;">{{ $name->transliteration }}</div>
                    <div style="font-size: 0.8rem; color: var(--text-light); margin-top: 5px;">{{ Str::limit($name->meaning_english, 20) }}</div>
                </div>
                @endforeach
            </div>
        </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 25px;">
                ...
                <div style="background: var(--white); border: 1px solid rgba(17, 70, 121, 0.1); border-radius: 12px; overflow: hidden; transition: var(--tr); box-shadow: 0 4px 15px rgba(0,0,0,0.03);">
                    <div style="padding: 25px;">
                        <span style="font-size: 0.8rem; color: var(--gold); font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">{{ $article->category->name ?? 'Knowledge' }}</span>
                        <h3 style="font-size: 1.3rem; margin: 10px 0; color: var(--text-dark); line-height: 1.4;">{{ $article->title }}</h3>
                        <p style="color: var(--text-light); font-size: 0.95rem; line-height: 1.6; margin-bottom: 20px;">{{ Str::limit($article->excerpt, 120) }}</p>
                        <a href="{{ route('blog.show', $article->slug) }}" style="color: var(--primary); font-weight: 600; text-decoration: none; display: inline-flex; align-items: center; gap: 8px;">Read Article <i class="fas fa-arrow-right" style="font-size: 0.8rem;"></i></a>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px;">
                ...
                <div style="background: var(--white); padding: 20px; border-radius: 10px; display: flex; align-items: center; gap: 20px; border-left: 4px solid var(--primary); box-shadow: 0 4px 10px rgba(0,0,0,0.03);">
                    <div style="background: var(--primary-light); color: var(--primary); padding: 10px; border-radius: 8px; text-align: center; min-width: 60px;">
                        <div style="font-size: 1.2rem; font-weight: 700;">{{ $event->hijri_day }}</div>
                        <div style="font-size: 0.8rem; font-weight: 600; text-transform: uppercase;">{{ substr($event->hijri_month, 0, 3) }}</div>
                    </div>
                    <div>
                        <h3 style="font-size: 1.1rem; color: var(--text-dark); margin-bottom: 5px;">{{ $event->name }}</h3>
                        <p style="font-size: 0.85rem; color: var(--text-light); margin: 0;"><i class="far fa-calendar-alt" style="margin-right: 5px; color: var(--gold);"></i> Expected: {{ \Carbon\Carbon::parse($event->estimated_gregorian_date)->format('M d, Y') }}</p>
                    </div>
                </div>
                @endforeach
            </div>
    ...
    <section class="section contact-section" id="contact">
        <div class="section-inner">
            ...
            <div style="text-align: center; max-width: 600px; margin: 0 auto; padding: 40px; background: white; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
                ...
                <a href="{{ route('contact') }}" class="btn-primary" style="display: inline-block;"><i class="fas fa-envelope"></i> Send a Message</a>
            </div>
        </div>
    </section>
