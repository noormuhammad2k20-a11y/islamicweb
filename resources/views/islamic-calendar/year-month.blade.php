...
<section class="date-hero">
    <h1 class="date-hero-title">Islamic Calendar {{ $monthName }} {{ $year }}</h1>
    <p class="date-hero-subtitle">Complete Hijri Calendar for {{ $monthName }} {{ $year }}</p>
</section>
...
<section class="section-container" id="calendar-print-area">
    <div class="title-wrapper" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
        <h2 class="section-title" style="margin-bottom: 0;">Islamic Calendar {{ $monthName }} {{ $year }}</h2>
...
<section class="section-container">
    <div class="internal-links">
        ...
        <a href="{{ route('islamic-calendar-year', $year) }}" class="internal-link">🗓️ Full Year {{ $year }}</a>
...
<section class="section-container">
    <div class="title-wrapper">
        <h2 class="section-title">FAQ — Islamic Calendar {{ $monthName }} {{ $year }}</h2>
    </div>
    @php
    $faqs = [
...
<section class="section-container">
    <div class="seo-content">
        <h2>Islamic Calendar {{ $monthName }} {{ $year }}</h2>
        <p>The <strong>Islamic calendar for {{ $monthName }} {{ $year }}</strong> helps you easily find the Hijri date for any Gregorian date in {{ $monthName }}. Since the Islamic calendar is lunar, the Hijri dates shift relative to the Gregorian calendar each year.</p>
        <p>You can print this monthly calendar or check other months by visiting the <a href="{{ route('islamic-calendar-year', $year) }}">Full {{ $year }} Islamic Calendar</a> page.</p>
    </div>
</section>
