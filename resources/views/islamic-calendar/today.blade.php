...
{{-- HERO — GIANT DATE DISPLAY --}}
<section class="date-hero">
    <h1 class="date-hero-title">Islamic Date Today {{ $nowPK->format('d F Y') }} | آج کی اسلامی تاریخ</h1>
    <p class="date-hero-subtitle">Exact Islamic Hijri Date — Pakistan, Saudi Arabia, All Cities</p>

    <div class="giant-date-display">
        <div class="giant-day">{{ $hijriPK['day'] }}</div>
        <div class="giant-month">{{ $hijriPK['month_name'] }} {{ $hijriPK['year'] }} AH</div>
        <div class="giant-year">{{ $hijriPK['month_urdu'] }} — {{ $hijriPK['month_arabic'] }}</div>
    </div>
...
<section class="section-container">
    ...
    <div class="progress-section">
        <div class="progress-card">
            <div class="progress-label">📅 Days Passed in Hijri Year {{ $hijriPK['year'] }} AH</div>
            <div class="progress-bar-bg">
                <div class="progress-bar-fill progress-bar-green" style="width: {{ $yearPercent }}%"></div>
            </div>
            <div class="progress-value">~{{ round($daysPassed) }} of ~{{ $hijriDaysInYear }} days ({{ $yearPercent }}%)</div>
        </div>

        <div class="progress-card">
            <div class="progress-label">🌙 Days Remaining in {{ $hijriPK['month_name'] }}</div>
            <div class="progress-bar-bg">
                <div class="progress-bar-fill progress-bar-gold" style="width: {{ $monthPercent }}%"></div>
            </div>
            <div class="progress-value">{{ $daysRemainingMonth }} days remaining (Day {{ $hijriPK['day'] }} of ~{{ $hijriMonthDays }})</div>
        </div>
    </div>
</section>
...
<section class="section-container">
    <div class="tomorrow-card">
        ...
        <div class="tomorrow-date">{{ $hijriTomorrow['formatted'] }}</div>
        <div style="color: #666; margin-top: 8px;">{{ $hijriTomorrow['month_urdu'] }} — {{ $hijriTomorrow['day_name'] }}</div>
    </div>
</section>
...
<section class="section-container">
    ...
    <table class="cities-table">
        ...
        <tbody>
            @foreach($citiesData as $cityName => $hijri)
                <tr>
                    <td><strong><a href="{{ route('islamic-date-city', strtolower(str_replace(' ', '-', $cityName))) }}" style="color: var(--primary); text-decoration: none;">{{ $cityName }}</a></strong></td>
                    <td>{{ $hijri['day'] }} {{ $hijri['month_name'] }} {{ $hijri['year'] }} AH</td>
                    <td>{{ $hijri['month_name'] }}</td>
                    <td style="font-family: 'Amiri', serif;">{{ $hijri['month_urdu'] }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
...
<section class="section-container">
    <div class="title-wrapper">
        <h2 class="section-title">{{ $monthContent->month_name_en ?? $hijriPK['month_name'] }} {{ $hijriPK['year'] }} — Current Islamic Month</h2>
    </div>
    <div style="background: white; padding: 30px; border-radius: 16px; border: 1px solid var(--border-light);">
        <p>{{ Str::limit($monthContent->significance_en ?? '', 500) }}</p>
        <a href="{{ route('islamic-month', $monthContent->slug ?? 'muharram') }}" style="color: var(--gold); font-weight: 700; text-decoration: none;">Read more about {{ $monthContent->month_name_en ?? $hijriPK['month_name'] }} →</a>
    </div>
</section>
@endif
...
<section class="section-container">
    ...
    <div class="internal-links">
        <a href="{{ route('islamic-calendar') }}" class="internal-link">📅 Full Islamic Calendar</a>
        ...
        <a href="{{ route('islamic-date-city', 'karachi') }}" class="internal-link">🏙️ Karachi</a>
        <a href="{{ route('islamic-date-city', 'lahore') }}" class="internal-link">🏙️ Lahore</a>
        <a href="{{ route('islamic-date-city', 'rawalpindi') }}" class="internal-link">🏙️ Rawalpindi</a>
        <a href="{{ route('islamic-date-city', 'faisalabad') }}" class="internal-link">🏙️ Faisalabad</a>
    </div>
</section>
...
<section class="section-container">
    <div class="seo-content">
        ...
        <p>The current Islamic month is <strong>{{ $hijriPK['month_name'] }}</strong> ({{ $hijriPK['month_urdu'] }}), which is the {{ $hijriPK['month'] }}th month of the Hijri year {{ $hijriPK['year'] }}. Tomorrow's Islamic date will be <strong>{{ $hijriTomorrow['formatted'] }}</strong>.</p>
    </div>
</section>
