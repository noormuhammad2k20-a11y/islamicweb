...
<section class="date-hero">
    ...
    @include('islamic-calendar.partials._date-card', ['hijriPK' => $hijriPK, 'hijriSA' => $hijriPK, 'nowPK' => $nowPK])
</section>
...
<section class="section-container">
    ...
    <div class="province-grid">
        @foreach($provinces as $provName => $provData)
            <div class="province-card">
                <div class="province-name">{{ $provName }}</div>
                <div style="font-size: 1.1rem; font-weight: 600; color: var(--gold); margin-bottom: 12px;">
                    {{ $hijriPK['formatted'] }}
                </div>
                <div class="province-cities">
                    @foreach($provData['cities'] as $cityName)
                        @php $citySlug = strtolower(str_replace(' ', '-', $cityName)); @endphp
                        <a href="{{ route('islamic-date-city', $citySlug) }}" class="province-city-tag">{{ $cityName }}</a>
                    @endforeach
                </div>
            </div>
        @endforeach
...
<section class="section-container">
    <div class="info-box">
        ...
        <p>Today's official Islamic date in Pakistan: <strong>{{ $hijriPK['formatted'] }}</strong></p>
    </div>
</section>

{{-- WHY PAKISTAN DATE DIFFERS --}}
<section class="section-container">
    <div class="info-box" style="background: white; border-color: var(--border-light);">
        ...
        <p>Currently: Pakistan is on <strong>{{ $hijriPK['formatted'] }}</strong>. This date applies to all cities in Pakistan including Karachi, Lahore, Islamabad, Rawalpindi, Faisalabad, Peshawar, Quetta, and Multan.</p>
    </div>
</section>
...
<section class="section-container">
    <div class="seo-content">
        <h2>Islamic Date Today in Pakistan — Complete Guide {{ $nowPK->format('Y') }}</h2>
        <p><strong>Islamic date today in Pakistan</strong> is <strong>{{ $hijriPK['formatted'] }}</strong>. Pakistan, officially the Islamic Republic of Pakistan, has a population of over 230 million Muslims who follow the Islamic Hijri calendar for religious observances. The <strong>today Islamic date in Pakistan</strong> is determined by the Central Ruet-e-Hilal Committee and applies uniformly across all provinces — Punjab, Sindh, Khyber Pakhtunkhwa, Balochistan, Azad Jammu & Kashmir, Gilgit-Baltistan, and the Islamabad Capital Territory.</p>

        <h3>What Is Islamic Date Today in Pakistan?</h3>
        <p><strong>What is Islamic date today in Pakistan</strong>? The answer is {{ $hijriPK['day'] }} {{ $hijriPK['month_name'] }} {{ $hijriPK['year'] }} AH ({{ $nowPK->format('d F Y') }}). This date is the same for all Pakistan cities. The <strong>which Islamic date is today in Pakistan</strong> query is answered by our real-time converter that uses Pakistan Standard Time (UTC+5) for accurate calculations.</p>

        <h3>Today Pakistan Islamic Date — All Cities</h3>
        <p><strong>Today Pakistan Islamic date</strong> applies to Karachi (Sindh), Lahore (Punjab), Islamabad (ICT), Rawalpindi (Punjab), Faisalabad (Punjab), Peshawar (KPK), Quetta (Balochistan), and Multan (Punjab). All these cities observe the same Islamic date: {{ $hijriPK['formatted'] }}. The <strong>today date Islamic in Pakistan</strong> is updated daily at midnight PKT.</p>

        <h3>Islamic Date Today in Pakistan — Madani Channel</h3>
        <p>Many Muslims check <strong>Islamic date today in Pakistan Madani Channel</strong> for the official Hijri date. Madani Channel follows the Ruet-e-Hilal Committee's announcements, same as our page. The official Pakistan Islamic date today is {{ $hijriPK['formatted'] }}.</p>
    </div>
</section>
