...
<section class="date-hero">
    <h1 class="date-hero-title">{{ $content->month_name_en }} {{ $hijriPK['year'] }} | {{ $content->month_name_urdu }}</h1>
    <p class="date-hero-subtitle">Islamic Month {{ $content->month_name_en }} — {{ $content->month_name_arabic }}</p>

    <div class="month-hero-card">
        <div class="month-number-badge">{{ $content->month_number }}</div>
        <div class="month-name-en">{{ $content->month_name_en }}</div>
        <div class="month-name-ur">{{ $content->month_name_urdu }}</div>
        <div class="month-name-ar">{{ $content->month_name_arabic }}</div>
    </div>
...
<section class="section-container">
    ...
    <div class="months-nav">
        @foreach($allMonths as $num => $m)
            <a href="{{ route('islamic-month', $m['slug']) }}" class="{{ $content->slug === $m['slug'] ? 'active' : '' }}">{{ $m['name'] }}</a>
        @endforeach
    </div>
</section>
...
<section class="section-container">
    <div class="title-wrapper">
        <h2 class="section-title">Significance of {{ $content->month_name_en }}</h2>
    </div>
    <div class="month-content-card">
        <h3>📖 About {{ $content->month_name_en }} ({{ $content->month_name_urdu }})</h3>
        <p>{{ $content->significance_en }}</p>
    </div>
</section>
...
<section class="section-container" id="calendar">
    <div class="title-wrapper">
        <h2 class="section-title">{{ $content->month_name_en }} Calendar {{ $hijriYear }}</h2>
    </div>
    
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 15px;">
        <p style="color: #666; font-size: 1.05rem; margin: 0;">
            Below is the complete Hijri calendar specifically for <strong>{{ $content->month_name_en }} {{ $hijriYear }}</strong>.
        </p>
        <form method="GET" action="{{ route('islamic-month', $content->slug) }}#calendar" style="display: flex; gap: 10px; align-items: center; background: white; padding: 10px 15px; border-radius: 12px; border: 1px solid var(--border-light);">
            ...
            <select name="year" id="year" onchange="this.form.submit()" style="padding: 6px 12px; border-radius: 8px; border: 1px solid #ccc; font-weight: 600; color: #333; outline: none; cursor: pointer;">
                @for($y = $hijriPK['year'] - 2; $y <= $hijriPK['year'] + 3; $y++)
                    <option value="{{ $y }}" {{ $hijriYear == $y ? 'selected' : '' }}>{{ $y }} AH</option>
                @endfor
            </select>
        </form>
    </div>

    @if(!empty($hijriMonthGrid))
...
@endphp
@if($importantDates && count($importantDates) > 0)
<section class="section-container">
    <div class="title-wrapper">
        <h2 class="section-title">Important Dates in {{ $content->month_name_en }}</h2>
    </div>
    <div class="important-dates-grid">
        @foreach($importantDates as $iDate)
            <div class="date-item">
                <div class="date-num">{{ $iDate['date'] ?? '—' }}</div>
                <div class="date-event">{{ $iDate['event'] ?? '' }}</div>
            </div>
        @endforeach
    </div>
</section>
...
{{-- RECOMMENDED IBADAH --}}
@if($content->recommended_ibadah)
<section class="section-container">
    <div class="month-content-card" style="background: linear-gradient(135deg, #fdf6e3, #fefcf2); border-color: var(--gold);">
        <h3>🤲 Recommended Ibadah in {{ $content->month_name_en }}</h3>
        <p>{{ $content->recommended_ibadah }}</p>
    </div>
</section>
@endif

{{-- HADITH --}}
@if($content->hadith_about_month)
<section class="section-container">
    <div class="month-content-card">
        <h3>📜 Hadith About {{ $content->month_name_en }}</h3>
        <p style="font-style: italic; border-left: 4px solid var(--gold); padding-left: 20px;">{{ $content->hadith_about_month }}</p>
    </div>
</section>
@endif

{{-- URDU SIGNIFICANCE --}}
@if($content->significance_urdu)
<section class="section-container">
    <div class="month-content-card" style="direction: rtl; font-family: 'Amiri', serif;">
        <h3 style="font-family: 'Amiri', serif;">{{ $content->month_name_urdu }} کی فضیلت</h3>
        <p style="line-height: 2.2; font-size: 1.05rem;">{{ $content->significance_urdu }}</p>
    </div>
</section>
@endif
...
<section class="section-container">
    ...
    <div class="internal-links" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 15px; margin-top: 20px;">
        <a href="{{ route('islamic-calendar') }}" class="internal-link" style="display: flex; align-items: center; gap: 15px; padding: 18px; border-radius: 14px; background: white; border: 1px solid var(--border-light); text-decoration: none; color: var(--primary); font-weight: 700; transition: all 0.3s; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
        ...
        <a href="{{ route('islamic-date-today') }}" class="internal-link" style="display: flex; align-items: center; gap: 15px; padding: 18px; border-radius: 14px; background: white; border: 1px solid var(--border-light); text-decoration: none; color: var(--primary); font-weight: 700; transition: all 0.3s; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
        ...
        <a href="{{ route('islamic-date-pakistan') }}" class="internal-link" style="display: flex; align-items: center; gap: 15px; padding: 18px; border-radius: 14px; background: white; border: 1px solid var(--border-light); text-decoration: none; color: var(--primary); font-weight: 700; transition: all 0.3s; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
        ...
        <a href="{{ route('islamic-date-saudi') }}" class="internal-link" style="display: flex; align-items: center; gap: 15px; padding: 18px; border-radius: 14px; background: white; border: 1px solid var(--border-light); text-decoration: none; color: var(--primary); font-weight: 700; transition: all 0.3s; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
            <div style="font-size: 1.8rem; color: var(--gold); background: rgba(10,58,42,0.04); width: 45px; height: 45px; display: flex; align-items: center; justify-content: center; border-radius: 10px;">🇸🇦</div>
            <div>Saudi Arabia</div>
        </a>
        <a href="{{ route('prayer-times.hub') }}" class="internal-link" style="display: flex; align-items: center; gap: 15px; padding: 18px; border-radius: 14px; background: white; border: 1px solid var(--border-light); text-decoration: none; color: var(--primary); font-weight: 700; transition: all 0.3s; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
        ...
        <a href="{{ route('tools.qibla') }}" class="internal-link" style="display: flex; align-items: center; gap: 15px; padding: 18px; border-radius: 14px; background: white; border: 1px solid var(--border-light); text-decoration: none; color: var(--primary); font-weight: 700; transition: all 0.3s; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
        ...
        <a href="{{ route('ramadan.timetable') }}" class="internal-link" style="display: flex; align-items: center; gap: 15px; padding: 18px; border-radius: 14px; background: white; border: 1px solid var(--border-light); text-decoration: none; color: var(--primary); font-weight: 700; transition: all 0.3s; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
...
<section class="section-container">
    <div class="title-wrapper">
        <h2 class="section-title">FAQ — {{ $content->month_name_en }}</h2>
    </div>
    @php
    $faqs = [
...
<section class="section-container">
    <div class="seo-content">
        <h2>{{ $content->month_name_en }} {{ $hijriPK['year'] }} — Complete Islamic Month Guide</h2>
        <p><strong>{{ $content->month_name_en }}</strong> ({{ $content->month_name_urdu }} / {{ $content->month_name_arabic }}) is the {{ $content->month_number }}th month of the Islamic Hijri calendar. This month holds special significance in Islam with unique events, recommended prayers, and historical importance. {{ Str::limit($content->significance_en, 250) }}</p>

        <h3>{{ $content->month_name_en }} in the Islamic Calendar</h3>
        <p>The Islamic calendar year begins with Muharram and ends with Dhu al-Hijjah. {{ $content->month_name_en }} is the {{ $content->month_number }}th month, occurring after {{ $content->month_number > 1 ? $allMonths[$content->month_number - 1]['name'] : 'Dhu al-Hijjah' }} and before {{ $content->month_number < 12 ? $allMonths[$content->month_number + 1]['name'] : 'Muharram' }}. Each Islamic month has 29 or 30 days, determined by moon sighting.</p>
    </div>
</section>
