...
<section class="date-hero">
    <h1 class="date-hero-title">Islamic Calendar {{ $year }} | Hijri Calendar {{ $startHijri['year'] }}–{{ $endHijri['year'] }} AH</h1>
    <p class="date-hero-subtitle">Complete Islamic Calendar for {{ $year }} with all 12 months, Ramadan, Eid dates</p>
</section>
...
<section class="section-container">
    <div class="hijri-span-card">
        ...
        <div class="hijri-span-value">{{ $year }} covers {{ $startHijri['year'] }}–{{ $endHijri['year'] }} AH</div>
        <div style="color: var(--text-medium); margin-top: 8px;">January {{ $year }}: {{ $startHijri['formatted'] }} — December {{ $year }}: {{ $endHijri['formatted'] }}</div>
    </div>
</section>
...
<section class="section-container">
    <div class="year-nav">
        @for($y = 2018; $y <= 2036; $y++)
            <a href="{{ route('islamic-calendar-year', $y) }}" class="year-nav-link {{ $y == $year ? 'active' : '' }}">{{ $y }}</a>
        @endfor
    </div>
</section>
...
<section class="section-container" id="calendar-print-area">
    <div class="title-wrapper" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
        <h2 class="section-title" style="margin-bottom: 0;">Full Islamic Calendar {{ $year }}</h2>
...
<section class="section-container">
    <div class="title-wrapper">
        <h2 class="section-title">Islamic Events in {{ $year }}</h2>
    </div>
    <div class="events-timeline">
        @foreach($yearEvents as $event)
            <div class="event-card">
                <div class="event-icon event-icon-{{ $event->event_type }}">
                    @switch($event->event_type) @case('eid') 🌙 @break @case('ramadan') 🕌 @break @case('hajj') 🕋 @break @case('muharram') 📿 @break @default ☪️ @endswitch
                </div>
                <div>
                    <div class="event-name">{{ $event->event_name }}</div>
                    <div class="event-date">{{ $event->hijri_date ?? '' }} @if($event->gregorian_date) · {{ $event->gregorian_date->format('d M Y') }} @endif</div>
                </div>
            </div>
        @endforeach
    </div>
...
<section class="section-container">
    <div class="internal-links">
        <a href="{{ route('islamic-calendar') }}" class="internal-link">📅 Current Calendar</a>
        <a href="{{ route('islamic-date-today') }}" class="internal-link">📅 Date Today</a>
        <a href="{{ route('islamic-date-pakistan') }}" class="internal-link">🇵🇰 Pakistan</a>
        <a href="{{ route('islamic-date-saudi') }}" class="internal-link">🇸🇦 Saudi</a>
        @if($year > 2018)<a href="{{ route('islamic-calendar-year', $year - 1) }}" class="internal-link">← {{ $year - 1 }}</a>@endif
        @if($year < 2036)<a href="{{ route('islamic-calendar-year', $year + 1) }}" class="internal-link">{{ $year + 1 }} →</a>@endif
    </div>
</section>
...
<section class="section-container">
    <div class="title-wrapper">
        <h2 class="section-title">FAQ — Islamic Calendar {{ $year }}</h2>
    </div>
    @php
    $faqs = [
...
<section class="section-container">
    <div class="seo-content">
        <h2>Islamic Calendar {{ $year }} — Complete Hijri Calendar {{ $startHijri['year'] }}–{{ $endHijri['year'] }} AH</h2>
        <p>The <strong>Islamic calendar {{ $year }}</strong> covers Hijri years {{ $startHijri['year'] }} to {{ $endHijri['year'] }} AH. This page displays the complete 12-month calendar for {{ $year }} with both Gregorian and Hijri dates side by side. Each month shows the corresponding Islamic date, making it easy to track when Ramadan, Eid ul-Fitr, Eid ul-Adha, Muharram, and other Islamic events fall in {{ $year }}.</p>

        <h3>Islamic Calendar {{ $year }} Today Date</h3>
        <p>The <strong>Islamic calendar {{ $year }} today date</strong> in Pakistan is shown in the calendar grid above. January 1, {{ $year }} corresponds to {{ $startHijri['formatted'] }}, and December 31, {{ $year }} corresponds to {{ $endHijri['formatted'] }}. The Hijri calendar is approximately 10-11 days shorter than the Gregorian calendar, so two Hijri years typically overlap in a single Gregorian year.</p>

        <h3>Islamic Events in {{ $year }}</h3>
        <p>Major Islamic events in {{ $year }} include Ramadan (9th month), Eid ul-Fitr (1 Shawwal), Eid ul-Adha (10 Dhu al-Hijjah), Muharram (1st month with Ashura on 10th), Shab-e-Meraj (27 Rajab), Shab-e-Barat (15 Shaban), and Eid Milad-un-Nabi (12 Rabi al-Awwal). All these events are marked in the calendar above with color-coded badges.</p>
    </div>
</section>
