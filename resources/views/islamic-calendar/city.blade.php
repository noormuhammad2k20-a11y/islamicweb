...
<section class="date-hero">
    <h1 class="date-hero-title">Islamic Date Today in {{ $cityName }} | آج کی اسلامی تاریخ {{ $cityName }}</h1>
    <p class="date-hero-subtitle">Exact Hijri Date — {{ $cityName }}, Pakistan — {{ $hijri['formatted'] }}</p>

    @include('islamic-calendar.partials._date-card', ['hijriPK' => $hijri, 'hijriSA' => $hijri, 'nowPK' => $nowPK])
</section>

{{-- CITY ISLAMIC HISTORY --}}
@if($cityContent && $cityContent->islamic_history)
<section class="section-container">
    <div class="city-info-card">
        <h3>🏛️ Islamic History of {{ $cityName }}</h3>
        <p>{{ $cityContent->islamic_history }}</p>
    </div>
</section>
@endif

{{-- FAMOUS MOSQUES --}}
@if($cityContent && $cityContent->famous_mosques)
<section class="section-container">
...
{{-- LOCAL EVENTS --}}
@if($cityContent && $cityContent->local_events)
<section class="section-container">
    <div class="city-info-card">
        <h3>📅 Local Islamic Events in {{ $cityName }}</h3>
        <p>{{ $cityContent->local_events }}</p>
    </div>
</section>
@endif
...
<section class="section-container">
    <div class="city-info-card" style="background: linear-gradient(135deg, #fdf6e3, #fefcf2); border-color: var(--gold);">
        <h3>🕐 Prayer Times in {{ $cityName }}</h3>
        <p>Check today's prayer times (Namaz timings) for {{ $cityName }} including Fajr, Dhuhr, Asr, Maghrib, and Isha.</p>
        @php $prayerSlug = strtolower(str_replace(' ', '-', $cityName)); @endphp
        <a href="{{ url("/prayer-times/{$prayerSlug}") }}" style="display: inline-block; margin-top: 10px; padding: 10px 25px; background: var(--primary); color: white; border-radius: 12px; text-decoration: none; font-weight: 700; transition: all 0.3s;">View Prayer Times {{ $cityName }} →</a>
    </div>
</section>
...
<section class="section-container">
    <div class="internal-links">
        ...
        <a href="{{ route('islamic-date-city', 'islamabad') }}" class="internal-link">🏙️ Islamabad</a>
...
<section class="section-container">
    <div class="title-wrapper">
        <h2 class="section-title">FAQ — Islamic Date {{ $cityName }}</h2>
    </div>
    @php
    $faqs = [
...
<section class="section-container">
    <div class="seo-content">
        <h2>Islamic Date Today in {{ $cityName }} — Hijri Date Guide</h2>
        <p><strong>Islamic date today in {{ $cityName }}</strong> is <strong>{{ $hijri['formatted'] }}</strong> ({{ $nowPK->format('d F Y') }}). {{ $cityName }} is one of the major cities of Pakistan with a significant Muslim population. The <strong>today Islamic date in {{ $cityName }}</strong> follows Pakistan's official Hijri calendar as announced by the Central Ruet-e-Hilal Committee.</p>

        <p>Muslims in {{ $cityName }} observe all Islamic dates and events including Ramadan, Eid ul-Fitr, Eid ul-Adha, Muharram, Rabi ul-Awwal, and Shab-e-Meraj according to the official Pakistan calendar. The current Islamic month in {{ $cityName }} is {{ $hijri['month_name'] }} ({{ $hijri['month_urdu'] }}), {{ $hijri['year'] }} AH.</p>

        @if($cityContent && $cityContent->islamic_history)
        <h3>{{ $cityName }}'s Islamic Heritage</h3>
        <p>{{ Str::limit($cityContent->islamic_history, 300) }}</p>
        @endif
    </div>
</section>
