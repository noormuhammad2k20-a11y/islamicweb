...
<nav class="max-w-[1100px] mx-auto px-5 py-4 flex items-center text-sm font-medium text-gray-500" aria-label="Breadcrumb">
  <ol class="inline-flex items-center space-x-1 md:space-x-2">
    ...
    <li>
      <div class="flex items-center">
        <svg width="16" height="16" class="w-4 h-4 text-gray-400 mx-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
        <a href="/prayer-times/{{ $citySlug }}" class="hover:text-[var(--primary)] transition-colors">{{ $city->name }}</a>
      </div>
    </li>
    <li aria-current="page">
      <div class="flex items-center">
        <svg width="16" height="16" class="w-4 h-4 text-gray-400 mx-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
        <span class="text-[var(--primary)] font-bold bg-[rgba(26,54,93,0.05)] px-3 py-1 rounded-full border border-[rgba(26,54,93,0.1)]">{{ ucfirst($prayerName) }}</span>
      </div>
    </li>
  </ol>
</nav>
...
<section class="date-hero">
    <div class="hero-meta">
        ...
        <span>±? {{ $tz }}</span>
        <span>?? Lat: {{ number_format($city->latitude ?? $city->lat, 4) }}, Lng: {{ number_format($city->longitude ?? $city->lng, 4) }}</span>
    </div>

    <h1 class="date-hero-title">{{ ucfirst($prayerName) }} Prayer in {{ $name }}</h1>
    ...
    <div class="date-cards-wrapper">
        <div class="side-card">
            ...
            <div class="side-card-value">{{ now($tz)->format('d M, Y') }}</div>
            <div class="side-card-sub">{{ now($tz)->format('l') }}</div>
        </div>

        <div class="main-time-card">
            <div class="card-label">Today's {{ ucfirst($prayerName) }} Time</div>
            <div class="large-time">{{ $prayers[$prayerKey] ?? '--:--' }}</div>
            ...
            <div class="countdown-box">
                ...
                <span class="countdown-timer">{{ $next['countdown'] ?? '00:00:00' }}</span>
            </div>
        </div>

        <div class="side-card">
            <div class="side-card-title">Islamic Date</div>
            <div class="side-card-value">{{ $hijri['day'] }} {{ $hijri['month_name'] }}</div>
            <div class="side-card-sub">{{ $hijri['year'] }} AH</div>
        </div>
    </div>
</section>
    ...
    <div class="content-grid">
        ...
        <div>
            ...
            @else
                ...
                <x-islamic-knowledge-section title="Importance of {{ ucfirst($prayerName) }} Prayer" type="info">
                    <p>The <strong>{{ ucfirst($prayerName) }}</strong> prayer is an obligatory (Fard) prayer and holds immense significance in the daily life of a Muslim. Establishing the five daily prayers is the second pillar of Islam and serves as a direct link between the believer and Allah (SWT).</p>
                    <p>Performing {{ ucfirst($prayerName) }} on time not only fulfills a divine command but also purifies the soul, instills discipline, and protects the believer from evil deeds. The precise timing depends on the movement of the sun, and Muslims are encouraged to pray in congregation at the mosque whenever possible.</p>
                </x-islamic-knowledge-section>

                <x-islamic-knowledge-section title="Quranic Reference" type="quran" reference="Surah Taha (20:130)">
            ...
            <div class="flex items-center justify-between mb-6 border-b-2 border-[#D4AF37] pb-2 mt-8">
                <h2 class="font-playfair text-2xl md:text-3xl font-bold text-[#1A365D] m-0">Monthly Timetable ({{ now($tz)->format('F Y') }})</h2>
                <div class="flex gap-2">
                    ...
                    <a href="/prayer-times/{{ $citySlug }}" class="action-btn text-xs py-1.5 px-3 btn-gold">All Prayers</a>
                </div>
            </div>
                ...
                <table class="table-modern">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ $day['is_today'] ? 'today-row' : '' }}">
                            ...
                            <td class="font-bold text-lg">{{ $day[$prayerKey] ?? '--:--' }}</td>
                            @if(in_array($prayerName, ['fajr'])) <td class="text-gray-500">{{ $day['sunrise'] ?? '--:--' }}</td> @endif
                            @if(in_array($prayerName, ['maghrib'])) <td class="text-gray-500">{{ $day['isha'] ?? '--:--' }}</td> @endif
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="bg-gradient-to-br from-[#f8fafc] to-[#f1f5f9] rounded-2xl p-6 border border-[#e2e8f0] shadow-sm relative overflow-hidden mb-10">
                ...
                <p class="text-gray-600 text-sm mb-4 leading-relaxed">
                    Prayer times in <strong class="text-gray-800">{{ $city->name }}</strong> are calculated using the <strong class="text-gray-800">University of Islamic Sciences, Karachi</strong> method by default. This method is widely adopted across South Asia and parts of the Middle East.
                </p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-5">
                    ...
                    <div class="bg-white p-4 rounded-xl border border-[rgba(212,175,55,0.3)] shadow-sm flex flex-col items-center justify-center text-center relative overflow-hidden">
                        ...
                        <span class="text-xl font-bold text-[var(--gold)] font-playfair relative z-10">{{ number_format($qibla ?? 0, 1) }}° N</span>
                    </div>
                </div>
