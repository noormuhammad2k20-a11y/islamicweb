...
@section('title', 'Hadith by Topic — احادیث بموضوع | Noor-e-Islam')
@section('meta_description', 'Browse authentic Hadiths organized by topics. Read Arabic text with Urdu and English translations from Sahih Bukhari, Muslim, Tirmidhi and more.')
@section('canonical', url('/hadith'))
...
<div class="prayer-ticker">
    <div class="prayer-ticker-inner" style="justify-content: center; gap: 40px;">
        <div class="prayer-ticker-label">
            <i class="fas fa-list-ul"></i> Total Topics: {{ $stats['total_topics'] }}
        </div>
        <div class="prayer-ticker-label">
            <i class="fas fa-quote-right"></i> Total Hadiths: {{ $stats['total_hadiths'] }}
        </div>
        <div class="prayer-ticker-label">
            <i class="fas fa-book"></i> Authentic Collections: {{ $stats['total_collections'] }}
        </div>
        <div class="prayer-ticker-label">
            <i class="fas fa-users"></i> Key Narrators: {{ $stats['total_narrators'] }}
        </div>
    </div>
</div>
...
<section class="section">
    <div class="section-inner">
        ...
        <div class="topics-grid">
            ...
            <a href="{{ route('hadith.show', $topic->slug) }}" class="topic-card">
                <div class="topic-card-head">
                    <div class="topic-icon"><i class="fas fa-bookmark"></i></div>
                    <div class="topic-native">
                        @if($topic->topic_name_arabic)
                        <span class="topic-native-ar">{{ $topic->topic_name_arabic }}</span>
                        @endif
                        @if($topic->topic_name_urdu)
                        <span class="topic-native-ur">{{ $topic->topic_name_urdu }}</span>
                        @endif
                    </div>
                </div>
                
                <h3 class="topic-name">{{ $topic->topic_name }}</h3>
                <div class="topic-count-badge">{{ $topic->hadiths_count }} Hadiths</div>
                
                <p class="topic-desc">
                    {{ Str::limit($topic->introduction ?? $topic->content, 85) }}
                </p>
                ...
                <div class="topic-stats-bar">
                    <div class="topic-stats-track">
                        <div class="stat-sahih" style="width: {{ ($sahihCount / $total) * 100 }}%;" title="Sahih: {{ $sahihCount }}"></div>
                        <div class="stat-hasan" style="width: {{ ($hasanCount / $total) * 100 }}%;" title="Hasan: {{ $hasanCount }}"></div>
                        <div class="stat-daif" style="width: {{ ($daifCount / $total) * 100 }}%;" title="Daif: {{ $daifCount }}"></div>
                    </div>
                    <div class="topic-stats-labels">
                        <span>Sahih {{ $sahihCount }}</span>
                        <span>Hasan {{ $hasanCount }}</span>
                    </div>
                </div>
        ...
        <div class="topics-grid">
            ...
            <a href="{{ route('hadith.collection', $collection->slug) }}" class="collection-card">
                <div class="collection-head">
                    <div class="collection-icon-wrapper">
                        <i class="fas fa-book-open"></i>
                    </div>
                    @if($collection->name_ar)
                    <div class="collection-ar">{{ $collection->name_ar }}</div>
                    @endif
                </div>
                <h3 class="collection-name">{{ $collection->name_en }}</h3>
                <div class="collection-meta">
                    <div class="collection-count"><i class="fas fa-list-ol"></i> {{ $collection->hadiths_count }} Hadiths</div>
                </div>
                <p class="collection-desc">{{ Str::limit($collection->introduction, 90) }}</p>
        ...
        <div class="topics-grid" style="grid-template-columns: repeat(auto-fill, minmax(280px, 1fr));">
            ...
            <div class="narrator-card">
                ...
                <div class="narrator-info">
                    <h3 class="narrator-name">{{ $narrator->name_en }}</h3>
                    <div class="narrator-count"><i class="fas fa-quote-right"></i> {{ $narrator->hadiths_count }} Narrations</div>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div style="max-width: 800px; margin: 0 auto;">
            ...
            <div class="hadith-card">
                <div class="hadith-card-header">
                    ...
                    <span class="grade-badge grade-{{ strtolower($fhadith->sahih_grade ?? 'sahih') }}">
                        {{ $fhadith->sahih_grade ?? 'Sahih' }}
                    </span>
                </div>
                <div class="hadith-card-body">
                    ...
                    <p style="font-size: 1.1rem; line-height: 1.8; color: var(--text-dark);">
                        {{ Str::limit($fhadith->english_translation, 150) }}
                    </p>
                </div>
                <div class="hadith-reference">
                    <span><i class="fas fa-book"></i> {{ $fhadith->book_name }}</span>
                    <span><i class="fas fa-hashtag"></i> {{ $fhadith->reference }}</span>
                    <a href="{{ route('hadith.show', $fhadith->topic->slug) }}" style="color: var(--navy); font-weight: 600; margin-left: auto; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; font-size: .9rem;">Read Full <i class="fas fa-arrow-right"></i></a>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div style="margin-top: 60px; display: flex; flex-wrap: wrap; gap: 20px; justify-content: center;">
            <a href="{{ route('surah.index') }}" class="btn-outline-navy">Explore Quran</a>
            <a href="{{ route('islamic-calendar') }}" class="btn-outline-navy">Islamic Date</a>
            <a href="{{ route('prayer-times.hub') }}" class="btn-outline-navy">Prayer Times</a>
        </div>
