...
@section('title', 'Islamic Calendar & Events — Explore Hijri Months')
@section('meta_description', 'Explore the Islamic Hijri calendar. Learn about the 12 Islamic months, their significance, and major events like Ramadan, Hajj, and Muharram.')
@section('meta_keywords', 'islamic calendar, hijri calendar, islamic events, islamic months, ramadan, muharram, dhul hijjah')
            ...
            <div class="months-grid">
                ...
                @if(isset($months) && count($months) > 0)
                    @foreach($months as $month)
                        ...
                        @endphp
                        <a href="{{ route('events.month', $month->slug) }}" class="month-card {{ $isSacred ? 'sacred-card' : '' }}">
                            @if($isSacred)
                                <div class="sacred-badge">Sacred</div>
                            @endif
                            <div class="m-number">{{ $month->month_number }}</div>
                            <div class="m-arabic">{{ $month->name_ar }}</div>
                            <div class="m-english">{{ $month->name_en }}</div>
                            <div class="m-urdu">({{ $month->name_ur }})</div>
                ...
                @else
                    ...
                    @endphp
                    @foreach($fallbackMonths as $m)
                        @php
                            $isSacred = in_array($m['num'], [1, 7, 11, 12]);
                        @endphp
                        <a href="#" class="month-card {{ $isSacred ? 'sacred-card' : '' }}">
                            @if($isSacred)
                                <div class="sacred-badge">Sacred</div>
                            @endif
                            <div class="m-number">{{ $m['num'] }}</div>
                            <div class="m-arabic">{{ $m['ar'] }}</div>
                            <div class="m-english">{{ $m['en'] }}</div>
                            <div class="m-urdu">({{ $m['ur'] }})</div>
                            <div class="m-btn">View Events <i class="fas fa-arrow-right"></i></div>
                        </a>
                    @endforeach
                @endif
