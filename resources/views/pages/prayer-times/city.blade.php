            ...
            <div style="display: flex; justify-content: space-between; align-items: flex-end; flex-wrap: wrap; gap: 8px;">
                <div>
                        <nav aria-label="breadcrumb">
                            <ul class="custom-breadcrumb">
                                ...
                                <li><span class="active">{{ $city->name }}</span></li>
                            </ul>
                        </nav>
                        <h1 class="page-title">Prayer Times in {{ $city->name }}</h1>
                    </div>
                    <p class="page-subtitle" style="margin-top: 4px;">
                        {{ date('F Y') }} Timetable & Nawafil
                    </p>
                </div>
            </div>
                ...
                <div class="bento-card hero-widget">
                    ...
                    <div class="hero-content">
                        <div>
                            ...
                            <h2 style="font-size:1.05rem; color:rgba(255,255,255,0.95); font-weight:600; margin:0;">
                                {{ date('l, d M Y') }} &bull; {{ isset($hijriDate) && $hijriDate ? $hijriDate->hijri_day . ' ' . $hijriDate->hijri_month : date('d M') }}
                            </h2>
                        </div>
                    ...
                    <div class="prayer-timeline">
                        <div class="timeline-item {{ $nextPrayer == 'Fajr' ? 'active' : '' }}">
                            <div class="p-name">Fajr</div>
                            <div class="p-time">{{ \Carbon\Carbon::parse($todayPrayer->fajr)->format('h:i A') }}</div>
                        </div>
                        <div class="timeline-item dimmed {{ $nextPrayer == 'Sunrise' ? 'active' : '' }}">
                            <div class="p-name">Sunrise</div>
                            <div class="p-time">{{ \Carbon\Carbon::parse($todayPrayer->sunrise)->format('h:i A') }}</div>
                        </div>
                        <div class="timeline-item {{ $nextPrayer == 'Dhuhr' ? 'active' : '' }}">
                            <div class="p-name">Dhuhr</div>
                            <div class="p-time">{{ \Carbon\Carbon::parse($todayPrayer->dhuhr)->format('h:i A') }}</div>
                        </div>
                        <div class="timeline-item {{ $nextPrayer == 'Asr' ? 'active' : '' }}">
                            <div class="p-name">Asr</div>
                            <div class="p-time">{{ \Carbon\Carbon::parse($todayPrayer->asr)->format('h:i A') }}</div>
                        </div>
                        <div class="timeline-item {{ $nextPrayer == 'Maghrib' ? 'active' : '' }}">
                            <div class="p-name">Maghrib</div>
                            <div class="p-time">{{ \Carbon\Carbon::parse($todayPrayer->maghrib)->format('h:i A') }}</div>
                        </div>
                        <div class="timeline-item {{ $nextPrayer == 'Isha' ? 'active' : '' }}">
                            <div class="p-name">Isha</div>
                            <div class="p-time">{{ \Carbon\Carbon::parse($todayPrayer->isha)->format('h:i A') }}</div>
                        </div>
                    </div>
                </div>
                @endif
                            ...
                            <div class="grid-2" style="grid-template-columns: 1fr;">
                                <div class="mini-card icon-left">
                                    ...
                                    <div>
                                        ...
                                        <div class="value highlight">{{ $qiblaDegree ?? 'N/A' }}°</div>
                                    </div>
                                </div>
                                <div class="mini-card icon-left">
                                    ...
                                    <div>
                                        ...
                                        <div class="value">{{ $city->latitude }}, {{ $city->longitude }}</div>
                                    </div>
                                </div>
                                @if($city->province)
                                <div class="mini-card icon-left">
                                    ...
                                    <div>
                                        ...
                                        <div class="value">{{ $city->province }}</div>
                                    </div>
                                </div>
                                @endif
                            </div>
                        ...
                        <div class="bento-card h-100" style="height: 100%; margin-bottom: 0;">
                            ...
                            <div class="grid-4 h-100" style="align-content: flex-start;">
                                <div class="mini-card text-center">
                                    ...
                                    <div class="value">{{ $nawafil->ishraq ?? 'N/A' }}</div>
                                </div>
                                <div class="mini-card text-center">
                                    <i class="fas fa-cloud-sun"></i>
                                    <div class="title">Chasht</div>
                                    <div class="value">{{ $nawafil->chasht ?? 'N/A' }}</div>
                                </div>
                                <div class="mini-card text-center">
                                    <i class="fas fa-moon"></i>
                                    <div class="title">Awwabeen</div>
                                    <div class="value">{{ $nawafil->awwabeen ?? 'N/A' }}</div>
                                </div>
                                <div class="mini-card text-center">
                                    <i class="fas fa-star-and-crescent"></i>
                                    <div class="title">Tahajjud</div>
                                    <div class="value">{{ $nawafil->tahajjud ?? 'N/A' }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bento-card mt-3">
                    ...
                    <div class="timetable-list">
                        ...
                        <div class="timetable-row {{ $isToday ? 'today-row' : '' }}">
                            ...
                            <div class="t-col" data-label="Fajr">{{ \Carbon\Carbon::parse($pt->fajr)->format('h:i A') }}</div>
                            <div class="t-col" data-label="Sunrise" style="color: #94A3B8;">{{ \Carbon\Carbon::parse($pt->sunrise)->format('h:i A') }}</div>
                            <div class="t-col" data-label="Dhuhr">{{ \Carbon\Carbon::parse($pt->dhuhr)->format('h:i A') }}</div>
                            <div class="t-col" data-label="Asr">{{ \Carbon\Carbon::parse($pt->asr)->format('h:i A') }}</div>
                            <div class="t-col" data-label="Maghrib" style="color: {{ $isToday ? 'var(--primary-dark)' : 'var(--primary, #10B981)' }};">
                                {{ \Carbon\Carbon::parse($pt->maghrib)->format('h:i A') }}
                            </div>
                            <div class="t-col" data-label="Isha">{{ \Carbon\Carbon::parse($pt->isha)->format('h:i A') }}</div>
                        </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bento-card">
                    ...
                    <div class="grid-auto-fill">
                        <a href="{{ route('zakat.index') }}" class="compact-link-card">
                            <i class="fas fa-calculator"></i>
                            <span>Zakat Calculator</span>
                        </a>
                        <a href="{{ route('duas.index') }}" class="compact-link-card">
                            <i class="fas fa-hands"></i>
                            <span>Daily Duas</span>
                        </a>
                        <a href="{{ route('names.index') }}" class="compact-link-card">
                            <i class="fas fa-book"></i>
                            <span>99 Names</span>
                        </a>
                        <a href="{{ route('prayer-times.hub') }}" class="compact-link-card">
                            <i class="fas fa-globe"></i>
                ...
                <div class="bento-card mb-0">
                    ...
                    <div class="grid-auto-fill">
                        ...
                        <a href="{{ route('prayer-times.city', $nearby->slug) }}" class="compact-link-card">
