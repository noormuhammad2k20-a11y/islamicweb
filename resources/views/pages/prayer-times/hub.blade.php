...
@section('title', $seoMeta->title ?? 'Namaz Timing Pakistan — Prayer Times All Cities')
@section('meta_description', $seoMeta->description ?? '')

@section('og_meta')
<meta property="og:title" content="{{ $seoMeta->title ?? 'Prayer Times Pakistan' }}">
<meta property="og:description" content="{{ $seoMeta->description ?? '' }}">
<meta property="og:url" content="{{ route('prayer-times.hub') }}">
<meta property="og:type" content="website">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="{{ $seoMeta->title ?? 'Prayer Times Pakistan' }}">
...
<section class="prayer-hub-hero">
    ...
    <div class="hub-hero-inner">
        ...
        <p class="hub-hero-subtitle">
            Accurate daily Salat timings for {{ $cities->count() }}+ cities across Pakistan.
        ...
        <div class="hub-hijri-bar">
            ...
            <span class="hijri-text">{{ $hijriDate->hijri_day }} {{ $hijriDate->hijri_month }} {{ $hijriDate->hijri_year }} AH</span>
        </div>
        @endif
    </div>
</section>
...
<section class="hub-section" style="background: #F8FAFC; padding-bottom: 40px;">
    <div class="hub-container">
        ...
        @foreach($citiesByProvince as $province => $provinceCities)
        <div class="province-section">
            <div class="province-header">
                <i class="fas fa-map-marker-alt"></i>
                <h2>{{ $province }}</h2>
                <span class="city-count">{{ $provinceCities->count() }} cities</span>
            </div>
            <div class="cities-grid">
                @foreach($provinceCities as $cityItem)
                <a href="{{ route('prayer-times.city', $cityItem->slug) }}" class="city-link-card">
                    <i class="fas fa-mosque"></i>
                    <span>{{ $cityItem->name }}</span>
                    @if($cityItem->name_ur)
                    <span class="urdu-name">{{ $cityItem->name_ur }}</span>
                    @endif
                </a>
                @endforeach
            </div>
        ...
        <div class="seo-content-block">
            ...
            <p>
                ...
                <strong>{{ $cities->count() }}+ cities</strong> across all provinces including Punjab, Sindh, KPK, Balochistan,
