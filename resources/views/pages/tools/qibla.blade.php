...
<script type="application/ld+json">
{
  ...
  "itemListElement": [
    ...
    { "@@type": "ListItem", "position": 3, "name": "Qibla Direction", "item": "{{ url('/tools/qibla-direction') }}" }
    @if(isset($countryName)), { "@@type": "ListItem", "position": 4, "name": "{{ $countryName }}", "item": "{{ url('/tools/qibla-direction/'.Str::slug($countryName)) }}" } @endif
    @if(isset($stateName)), { "@@type": "ListItem", "position": 5, "name": "{{ $stateName }}", "item": "{{ url('/tools/qibla-direction/'.Str::slug($countryName).'/'.Str::slug($stateName)) }}" } @endif
    @if(isset($cityName)), { "@@type": "ListItem", "position": 6, "name": "{{ $cityName }}", "item": "{{ $seo['canonical'] }}" } @endif
  ]
}
</script>
    ...
    <div class="section-inner">
        <nav aria-label="breadcrumb" class="custom-breadcrumb">
            <ol>
                ...
                <li><i class="fas fa-chevron-right"></i></li>
                <li><a href="{{ url('/tools/qibla-direction') }}">Qibla Direction</a></li>
                @if(isset($countryName))
                    <li><i class="fas fa-chevron-right"></i></li>
                    <li><a href="{{ url('/tools/qibla-direction/'.Str::slug($countryName)) }}">{{ $countryName }}</a></li>
                @endif
                @if(isset($stateName))
                    <li><i class="fas fa-chevron-right"></i></li>
                    <li><a href="{{ url('/tools/qibla-direction/'.Str::slug($countryName).'/'.Str::slug($stateName)) }}">{{ $stateName }}</a></li>
                @endif
                @if(isset($cityName))
                    <li><i class="fas fa-chevron-right"></i></li>
        ...
        <div class="page-header">
            ...
            <p>Find the exact direction of the Kaaba from {{ $locationName ?? 'your location' }} with high-precision GPS and interactive maps.</p>
        </div>
        ...
        <div class="dashboard-panel">
            ...
            <div class="tool-card">
                ...
                <div class="search-bar">
                    <input type="text" id="loc-input" class="search-input" placeholder="Search by city or country..." value="{{ $locationName ?? '' }}">
                ...
                <div id="view-compass" style="text-align: center; padding: 20px 0;">
                    ...
                    <p id="qibla-status" style="color: var(--text-medium); font-weight: 500;">
                        @if(isset($bearing))
                            Qibla Direction: <strong>{{ round($bearing, 2) }}°</strong> from North.<br>
            ...
            <div style="display: flex; flex-direction: column; gap: 20px;">
                <div class="tool-card">
                    ...
                    <div class="stat-grid">
                        <div class="stat-box">
                            ...
                            <div class="stat-val" id="res-bearing">{{ isset($bearing) ? round($bearing, 2) . '°' : '--' }}</div>
                        </div>
                        <div class="stat-box">
                            <div class="stat-label">Distance</div>
                            <div class="stat-val" id="res-dist">{{ isset($distance) ? number_format($distance) . ' km' : '--' }}</div>
                        </div>
                        <div class="stat-box">
                            <div class="stat-label">Latitude</div>
                            <div class="stat-val" id="res-lat">{{ isset($lat) ? round($lat, 4) : '--' }}</div>
                        </div>
                        <div class="stat-box">
                            <div class="stat-label">Longitude</div>
                            <div class="stat-val" id="res-lon">{{ isset($lon) ? round($lon, 4) : '--' }}</div>
                        </div>
                    </div>
                ...
                <div class="tool-card">
                    ...
                    <div style="font-size: 0.9rem; color: var(--text-medium); line-height: 1.8;">
                        ...
                        <p><strong style="color: var(--navy);">Location Method:</strong> {{ isset($cityName) ? 'Geocoding API' : 'IP Fallback' }}</p>
        ...
        <div class="dashboard-panel" style="margin-top: 20px;">
            <div class="tool-card" style="grid-column: 1 / -1;">
                <h4 class="card-title"><i class="fas fa-clock"></i> Prayer Times in {{ $locationName ?? 'your area' }}</h4>
                @if(isset($prayerData))
                    <div style="background: var(--gold-tint); padding: 15px; border-radius: var(--radius-sm); margin-bottom: 20px; text-align: center; border: 1px solid rgba(201, 168, 76, 0.15);">
                        <strong style="color: var(--navy);">Hijri Date:</strong> {{ $prayerData['date']['hijri']['day'] }} {{ $prayerData['date']['hijri']['month']['en'] }} {{ $prayerData['date']['hijri']['year'] }}<br>
                        <strong style="color: var(--navy);">Gregorian:</strong> {{ $prayerData['date']['readable'] }}
                    </div>
                    <ul class="mosque-list" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; border: none;">
                        <li style="border: 1px solid var(--border-light); padding: 15px; border-radius: var(--radius-sm); display: flex; flex-direction: column; align-items: center; justify-content: center; background: var(--bg-main);">
                            <span style="margin-bottom: 5px;">Fajr</span> <strong>{{ $prayerData['timings']['Fajr'] }}</strong>
                        </li>
                        <li style="border: 1px solid var(--border-light); padding: 15px; border-radius: var(--radius-sm); display: flex; flex-direction: column; align-items: center; justify-content: center; background: var(--bg-main);">
                            <span style="margin-bottom: 5px;">Sunrise</span> <strong>{{ $prayerData['timings']['Sunrise'] }}</strong>
                        </li>
                        <li style="border: 1px solid var(--border-light); padding: 15px; border-radius: var(--radius-sm); display: flex; flex-direction: column; align-items: center; justify-content: center; background: var(--bg-main);">
                            <span style="margin-bottom: 5px;">Dhuhr</span> <strong>{{ $prayerData['timings']['Dhuhr'] }}</strong>
                        </li>
                        <li style="border: 1px solid var(--border-light); padding: 15px; border-radius: var(--radius-sm); display: flex; flex-direction: column; align-items: center; justify-content: center; background: var(--bg-main);">
                            <span style="margin-bottom: 5px;">Asr</span> <strong>{{ $prayerData['timings']['Asr'] }}</strong>
                        </li>
                        <li style="border: 1px solid var(--border-light); padding: 15px; border-radius: var(--radius-sm); display: flex; flex-direction: column; align-items: center; justify-content: center; background: var(--bg-main);">
                            <span style="margin-bottom: 5px;">Maghrib</span> <strong>{{ $prayerData['timings']['Maghrib'] }}</strong>
                        </li>
                        <li style="border: 1px solid var(--border-light); padding: 15px; border-radius: var(--radius-sm); display: flex; flex-direction: column; align-items: center; justify-content: center; background: var(--bg-main);">
                            <span style="margin-bottom: 5px;">Isha</span> <strong>{{ $prayerData['timings']['Isha'] }}</strong>
                        </li>
                    </ul>
                @else
        ...
        {{-- Location Details & Kaaba Info Cards --}}
        <div class="dashboard-panel" style="margin-top: 20px;">
            <div class="tool-card">
                ...
                <div style="font-size: 0.95rem; line-height: 2; color: var(--text-medium);">
                    <strong style="color: var(--navy);">Country:</strong> <span id="det-country">{{ $countryName ?? 'Unknown' }}</span><br>
                    <strong style="color: var(--navy);">Province/State:</strong> <span id="det-state">{{ $stateName ?? 'N/A' }}</span><br>
                    <strong style="color: var(--navy);">City:</strong> <span id="det-city">{{ $cityName ?? 'N/A' }}</span><br>
                    <strong style="color: var(--navy);">Timezone:</strong> {{ isset($prayerData['meta']['timezone']) ? $prayerData['meta']['timezone'] : 'Auto' }}<br>
                    <strong style="color: var(--navy);">Latitude:</strong> {{ isset($lat) ? round($lat, 4) : '--' }}<br>
                    <strong style="color: var(--navy);">Longitude:</strong> {{ isset($lon) ? round($lon, 4) : '--' }}<br>
        ...
        <div class="content-section">
            ...
            <div class="grid-3">
                <a href="{{ url('/tools/qibla-direction/pakistan') }}" class="country-card">Pakistan</a>
                <a href="{{ url('/tools/qibla-direction/india') }}" class="country-card">India</a>
                <a href="{{ url('/tools/qibla-direction/saudi-arabia') }}" class="country-card">Saudi Arabia</a>
                <a href="{{ url('/tools/qibla-direction/united-arab-emirates') }}" class="country-card">UAE</a>
                <a href="{{ url('/tools/qibla-direction/united-kingdom') }}" class="country-card">United Kingdom</a>
                <a href="{{ url('/tools/qibla-direction/united-states') }}" class="country-card">United States</a>
            </div>

            <h2 class="content-title">Related Resources</h2>
            <div style="display: flex; gap: 15px; flex-wrap: wrap; margin-top: 20px;">
                <a href="{{ url('/prayer-times') }}" class="btn-secondary"><i class="fas fa-clock"></i> Prayer Times</a>
                <a href="{{ url('/tools/ramadan-calendar-generator') }}" class="btn-secondary"><i class="fas fa-calendar-alt"></i> Ramadan Calendar</a>
                <a href="{{ url('/tools/islamic-event-finder') }}" class="btn-secondary"><i class="fas fa-star-and-crescent"></i> Islamic Events</a>
                <a href="{{ url('/islamic-calendar') }}" class="btn-secondary"><i class="fas fa-calendar-day"></i> Islamic Calendar</a>
                <a href="{{ url('/zakat-calculator') }}" class="btn-secondary"><i class="fas fa-calculator"></i> Zakat Calculator</a>
                <a href="{{ url('/hijri-converter') }}" class="btn-secondary"><i class="fas fa-sync-alt"></i> Hijri Converter</a>
                <a href="{{ url('/99-names-of-allah') }}" class="btn-secondary"><i class="fas fa-hand-holding-heart"></i> 99 Names of Allah</a>
                <a href="{{ url('/wazaif') }}" class="btn-secondary"><i class="fas fa-book-open"></i> Quran & Wazaif</a>
            </div>
        </div>
...
<script>
    ...
    let currentLat = {{ isset($lat) ? $lat : 'null' }};
    let currentLon = {{ isset($lon) ? $lon : 'null' }};
    let targetBearing = {{ isset($bearing) ? $bearing : 'null' }};
    let map = null;
    let polyline = null;
    let userMarker = null;
