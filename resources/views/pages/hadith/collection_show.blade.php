...
@section('title', $bookName . ' - Hadith Collection')
@section('meta_description', 'Read and explore authentic hadiths from ' . $bookName . ' (' . ($collectionModel->name_ar ?? '') . ').')
...
<script type="application/ld+json">
{
  ...
  "@type": "Book",
  "name": "{{ $bookName }}",
  "alternateName": "{{ $collectionModel->name_ar ?? '' }}",
  "author": {
    "@type": "Person",
    "name": "{{ $collectionModel->compiler ?? 'Unknown' }}"
  },
  "url": "{{ route('hadith.collection', $collectionModel->slug) }}",
...
<div class="page-header" style="background: var(--primary); color: white; padding: 40px 0; text-align: center;">
    <div class="container">
        <h1 style="color: white; margin-bottom: 10px;">{{ $bookName }} <span style="font-family: 'Amiri', serif;">{{ $collectionModel->name_ar ?? '' }}</span></h1>
        <p style="opacity: 0.8; margin-bottom: 0;">{{ $collectionModel->reliability ?? 'Authentic Hadith Collection' }}</p>
    </div>
</div>
    ...
    <div class="row">
        <div class="col-md-4">
            <div style="background: white; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); padding: 30px; margin-bottom: 30px;">
                ...
                @if($collectionModel->compiler)
                <p><strong>Compiler:</strong> {{ $collectionModel->compiler }}</p>
                @endif
                
                <p><strong>Total Hadiths:</strong> {{ $hadiths->total() }}</p>
                
                @if($collectionModel->reliability)
                <p><strong>Authenticity:</strong> {{ $collectionModel->reliability }}</p>
                @endif
            </div>
        </div>
        
        <div class="col-md-8">
            @if($collectionModel->introduction || $collectionModel->history)
            <div style="background: white; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); padding: 30px; margin-bottom: 30px;">
                ...
                <div style="line-height: 1.8; color: #444;">
                    @if($collectionModel->introduction)
                    <p>{!! nl2br(e($collectionModel->introduction)) !!}</p>
                    @endif
                    @if($collectionModel->history)
                    <h4 style="margin-top: 20px;">History</h4>
                    <p>{!! nl2br(e($collectionModel->history)) !!}</p>
                    @endif
                </div>
            </div>
            @endif
            
            <h3 style="margin-top: 40px; margin-bottom: 20px; color: var(--primary);">Hadiths from {{ $bookName }}</h3>
                ...
                <div class="hadith-card" style="background: white; border-radius: 12px; box-shadow: 0 5px 20px rgba(0,0,0,0.05); margin-bottom: 30px; overflow: hidden; border: 1px solid #eee;">
                    <div class="hadith-header" style="background: #f8f9fa; padding: 15px 25px; border-bottom: 1px solid #eee; display: flex; justify-content: space-between; align-items: center;">
                        <div class="hadith-meta">
                            <span class="badge" style="background: var(--primary); color: white; padding: 6px 12px; border-radius: 20px; font-size: 0.85rem; font-weight: 500;">{{ $bookName }}</span>
                    ...
                    <div class="hadith-body" style="padding: 25px;">
                        ...
                        <div class="translation-text" style="font-size: 1.1rem; line-height: 1.8; color: #444;">
                            <p><strong>Narrated {{ $hadith->narrator }}:</strong></p>
