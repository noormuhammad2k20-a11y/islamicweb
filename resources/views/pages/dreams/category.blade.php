...
<section style="background: linear-gradient(135deg, #1a1a3e 0%, #2d1b69 50%, #1a1a3e 100%); padding: 50px 0; text-align: center; color: #fff; position: relative; overflow: hidden;">
    ...
    <div style="max-width: 800px; margin: 0 auto; padding: 0 20px; position: relative; z-index: 1;">
        <span style="font-size: 3rem; display: block; margin-bottom: 12px;">{{ $category->icon ?? '🔮' }}</span>
        <h1 style="font-family: 'Amiri', serif; font-size: 2.6rem; margin-bottom: 12px; direction: rtl;">{{ $category->name_urdu }} کے خواب</h1>
        <p style="font-size: 1.1rem; opacity: 0.9; direction: rtl; font-family: 'Amiri', serif;">{{ $category->description ?? 'اس زمرے سے متعلق خوابوں کی اسلامی تعبیر' }}</p>
        <p style="font-size: 0.95rem; opacity: 0.7; margin-top: 6px;">Total Dreams: {{ $category->dream_symbols_count }}</p>

        {{-- Search Within Category --}}
        <form action="{{ url('/khwabon-ki-tabeer/' . $category->slug) }}" method="GET" style="margin-top: 24px; display: flex; max-width: 500px; margin-left: auto; margin-right: auto;">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="اس زمرے میں تلاش کریں..." style="flex: 1; padding: 14px 20px; border: none; border-radius: 10px 0 0 10px; font-size: 1rem; direction: rtl; font-family: 'Amiri', serif;">
...
<div style="max-width: 1200px; margin: 0 auto; padding: 40px 20px;">
    ...
    <nav aria-label="breadcrumb" style="background: #f8f9fa; padding: 12px 20px; border-radius: 8px; font-size: 0.9rem; margin-bottom: 30px; direction: rtl; display: flex; align-items: center; border: 1px solid #eee;">
        <a href="{{ url('/') }}" style="color: #1a6b42; text-decoration: none;">ہوم</a>
        <span style="margin: 0 6px;">/</span>
        <a href="{{ route('dreams.index') }}" style="color: #1a6b42; text-decoration: none;">خوابوں کی تعبیر</a>
        <span style="margin: 0 6px;">/</span>
        <span style="color: #666;">{{ $category->name_english }}</span>
    </nav>

    @if(request()->has('search'))
        {{-- Search Results Mode --}}
        <h2 style="font-family: 'Amiri', serif; font-size: 1.8rem; color: #1a1a3e; text-align: center; direction: rtl; margin-bottom: 24px;">تلاش کے نتائج: {{ request('search') }}</h2>
        ...
        @else
            <div style="text-align: center; padding: 60px 20px; color: #888;">
                ...
                <a href="{{ url('/khwabon-ki-tabeer/' . $category->slug) }}" style="display: inline-block; margin-top: 16px; padding: 10px 24px; background: #1a1a3e; color: #fff; border-radius: 8px; text-decoration: none;">واپس جائیں</a>
            </div>
        @endif
    ...
    @else
        ...
        {{-- POPULAR DREAMS --}}
        @if($popularDreams->count() > 0)
        <div style="margin-bottom: 50px;">
            <h2 class="section-title">🔥 مقبول ترین خواب (Popular)</h2>
            <div class="dreams-grid">
                @foreach($popularDreams as $symbol)
                    @include('pages.dreams.partials.dream_card', ['symbol' => $symbol])
                @endforeach
            </div>
        </div>
        ...
        {{-- FEATURED DREAMS --}}
        @if($featuredDreams->count() > 0)
        <div style="margin-bottom: 50px;">
            <h2 class="section-title">⭐ نمایاں خواب (Featured)</h2>
            <div class="dreams-grid">
                @foreach($featuredDreams as $symbol)
                    @include('pages.dreams.partials.dream_card', ['symbol' => $symbol])
                @endforeach
            </div>
        </div>
