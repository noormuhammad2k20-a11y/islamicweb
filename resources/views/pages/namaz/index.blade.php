...
@section('title', 'Learn Namaz (Salah) — Step-by-Step Prayer Guides')
@section('meta_description', 'Learn how to perform Namaz (Salah) correctly with our comprehensive step-by-step guides. Essential for beginners and those looking to perfect their prayer.')
@section('meta_keywords', 'learn namaz, how to pray salah, step by step namaz, islamic prayer guide, namaz rules, wudu guide')
        ...
        <div class="n-grid-inner">
            <div class="guides-grid">
                ...
                @if(isset($guides) && count($guides) > 0)
                    @foreach($guides as $guide)
                        <a href="#" class="guide-card">
                            <div class="g-header">
                                <div class="g-icon"><i class="fas {{ $guide->icon ?? 'fa-praying-hands' }}"></i></div>
                                <div>
                                    <h3 class="g-title">{{ $guide->title ?? $guide->name }}</h3>
                                    @if(isset($guide->type))
                                        <div class="g-badge">{{ $guide->type }}</div>
                                    @endif
                                </div>
                            </div>
                            <div class="g-desc">{{ Str::limit($guide->description ?? $guide->overview ?? $guide->content, 120) }}</div>
                ...
                @else
                    ...
                    @endphp
                    @foreach($fallbackGuides as $g)
                        <a href="#" class="guide-card">
                            <div class="g-header">
                                <div class="g-icon"><i class="fas {{ $g['icon'] }}"></i></div>
                                <div>
                                    <h3 class="g-title">{{ $g['title'] }}</h3>
                                    <div class="g-badge">{{ $g['badge'] }}</div>
                                </div>
                            </div>
