...
@section('seo')
<title>Ramadan {{ $year }} Hub — Sehri, Iftar, Ashras & Zakat | IslamicWeb</title>
<meta name="description" content="Complete Ramadan {{ $year }} calendar, sehri and iftar timings, Ashra duas, Zakat calculator, Fidyah/Kaffarah rules, and Lailatul Qadr guide.">
<link rel="canonical" href="{{ url('/ramadan/' . $year) }}">
@endsection
...
<section class="date-hero">
    <h1 class="date-hero-title">Ramadan {{ $year }} Global Hub</h1>
...
<section class="section-container">
    ...
    <div id="citiesContainer">
        ...
        <div class="country-section" data-country="{{ strtolower($countryName) }}">
            <h3 class="country-title">📍 {{ $countryName }}</h3>
            <div class="city-grid">
                @foreach($countryCities as $city)
                <a href="{{ route('ramadan.city', ['year' => $year, 'city' => $city->slug]) }}" class="city-card" data-city="{{ strtolower($city->name) }}">
                    <h3>{{ $city->name }}</h3>
    ...
    <div class="guides-grid">
        <a href="{{ route('ramadan.laylatul_qadr') }}" class="guide-card">
        ...
        <a href="{{ route('zakat.index') }}" class="guide-card">
        ...
        <a href="{{ route('ramadan.rules') }}" class="guide-card">
            <div class="guide-icon">📖</div>
        ...
        <a href="{{ route('ramadan.duas') }}" class="guide-card">
            <div class="guide-icon">🤲</div>
