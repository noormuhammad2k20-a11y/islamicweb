...
<section class="date-hero">
    <h1 class="date-hero-title">Ramadan {{ $year }} Hub</h1>
...
<section class="section-container">
    ...
    @foreach($groupedCities as $countryName => $countryCities)
    <div class="country-section">
        <h3 class="country-title">📍 {{ $countryName }}</h3>
        <div class="city-grid">
            @foreach($countryCities as $city)
            <a href="{{ route('ramadan.city', ['year' => $year, 'city' => $city->slug]) }}" class="city-card">
                <h3>{{ $city->name }}</h3>
    ...
    <div style="display: flex; gap: 20px; justify-content: center; flex-wrap: wrap; margin-bottom: 60px;">
        ...
        <a href="{{ route('ramadan.rules') }}" style="text-decoration: none; background: white; padding: 25px; border-radius: 16px; flex: 1; min-width: 250px; text-align: center; color: var(--text-dark); box-shadow: var(--card-shadow); border: 1px solid var(--border-light); transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-5px)'" onmouseout="this.style.transform='none'">
            <div style="font-size: 2.5rem; margin-bottom: 15px;">📜</div>
    ...
    <!-- SEO FAQ -->
    <div style="text-align: center;">
        <h2 class="section-title">Ramadan {{ $year }} FAQs</h2>
    </div>
    ...
    <div class="faq-section" itemscope itemtype="https://schema.org/FAQPage" style="max-width: 900px; margin: 0 auto; margin-bottom: 50px;">
        <div class="faq-item" itemscope itemprop="mainEntity" itemtype="https://schema.org/Question" style="background: white; padding: 25px; border-radius: 16px; margin-bottom: 20px; box-shadow: var(--card-shadow); border: 1px solid var(--border-light);">
            <h3 itemprop="name" style="font-size: 1.2rem; color: var(--primary); font-family: 'Playfair Display', serif; margin: 0 0 10px;">When will Ramadan {{ $year }} begin?</h3>
            <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
                <div itemprop="text" style="color: #555; line-height: 1.8;">
                    The exact start date of Ramadan {{ $year }} depends on the sighting of the new moon. Generally, the dates are calculated based on the global astronomical moon phases, but local moon sighting committees will make the final announcement in your country.
                </div>
            </div>
        </div>
