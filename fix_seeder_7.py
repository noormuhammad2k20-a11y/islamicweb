...
# Note that we match the exact string including the opening and closing single quotes
array_code = re.sub(r"('[a-z_]+'\s*=>\s*)'(.*?)'(?=[\n\s,]|$)", convert_to_double_quotes, array_code, flags=re.DOTALL)
