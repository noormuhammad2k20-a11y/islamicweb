...
$categories = [
    ...
    'Duas' => [
        ...
        'url_pattern' => '/duas/category-placeholder/{slug}', // We might need to resolve category
    ...
    'Hadiths' => [
        ...
        'note' => 'Routes are complex (/hadith/collection/chapter/number). Crawling a sample.'
    ],
    'Islamic Knowledge' => [
        'routes' => [
...
$markdown = "# Website SEO Inventory & Analysis\n\n";
$markdown .= "Generated on: " . date('Y-m-d H:i:s') . "\n";
$markdown .= "> **Note:** For dynamic sections with thousands of pages (like Names, Dreams, Hadiths), a representative sample of up to 5 URLs is crawled. Generating an exhaustive list for 20,000+ pages would make this document unreadable.\n\n";

foreach ($categories as $catName => $catData) {
    $markdown .= "## {$catName} (Total Pages: {$catData['total']})\n";
    $markdown .= "| URL | Page Title | Meta Description | H1 | Index Status | Canonical URL | Page Type | Notes |\n";
    $markdown .= "|---|---|---|---|---|---|---|---|\n";

    // Crawl specific routes
    if (!empty($catData['routes'])) {
        foreach ($catData['routes'] as $route) {
            $seo = getSeoData($route);
            $markdown .= "| `{$route}` | {$seo['Title']} | {$seo['Description']} | {$seo['H1']} | {$seo['Index']} | {$seo['Canonical']} | Static/Hub | {$seo['Notes']} |\n";
        }
    }

    // Crawl dynamic models
        ...
        if (class_exists($modelClass)) {
            ...
            foreach ($records as $record) {
                if ($modelClass === 'App\Models\Dua') {
                    // special logic for dua if needed, dua routes are /duas/{category}/{slug}
                ...
                $seo = getSeoData($url);
                $markdown .= "| `{$url}` | {$seo['Title']} | {$seo['Description']} | {$seo['H1']} | {$seo['Index']} | {$seo['Canonical']} | Dynamic | {$seo['Notes']} |\n";
            }
        }
    }
...
file_put_contents('website-pages-analysis.md', $markdown);
echo "Analysis completed. Written to website-pages-analysis.md\n";
