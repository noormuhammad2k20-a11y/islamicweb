...
# Now fix the arrays in json_encode(['...'])
def convert_json_array_items(m):
    items_str = m.group(1)
    
    # Simple replace of single quotes with double quotes for the items
    def convert_item(m2):
        val = m2.group(1)
        val = val.replace("\\'", "'")
        val = val.replace('"', '\\"')
    ...
    new_items_str = re.sub(r"'(.*?)'(?=[\s,\]]|$)", convert_item, items_str)
    return 'json_encode([' + new_items_str + '])'

array_code = re.sub(r"json_encode\(\[(.*?)\]\)", convert_json_array_items, array_code, flags=re.DOTALL)
